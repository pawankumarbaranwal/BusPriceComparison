...
public class BusesWithFares extends AppCompatActivity {
    ...
    private void initializeView() {
        ...
        TextView searchDate = (TextView) findViewById(R.id.tv_search_date);
    ...
    private List<BusDetails> getData() {
        ...
        if (bundle!=null)
        {
            sourceAndDestination=(String)bundle.get("SRCandDSN");
            date=(String)bundle.get("Date");
            Log.i("kkkkkkkkkkkk", sourceAndDestination + "");
        }
        //("Date", date.getText()+"");
        //paytmIntent.putExtra("SRCandDSN", source.getText()+" to "+destination.getText());

        //sourceAndDestination = i.get//busesList.get(1).getSourceCity() + " to " + busesList.get(1).getDestinationCity() + "  -  ";
        list.addAll(set);
    ...
    private List<BusDetails> getStaticData() {
        List<BusDetails> list = new ArrayList<BusDetails>();
